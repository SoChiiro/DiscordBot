...
client.on('messageCreate', message => {
  ...
  const command = message.content.substring(COMMAND_PREFIX.length).toLowerCase();
  ...
  // NEWS
  if ( command === 'news') {
    message.channel.send("Une prochaine mise à jour accueillera une nouvelle commande : **kushi** qui recommande des items de luxe, pour t'habiller gucci à la prochaine maj");
  }
  if ( command === 'new') {
    message.channel.send("Une prochaine mise à jour accueillera une nouvelle commande : **kushi** qui recommande des items de luxe, pour t'habiller gucci à la prochaine maj");
  }
...
client.on('messageCreate', message => {
  ...
  if (command === 'howpd') {
    ...
    if (randomNumber === 0) {
      ...
      message.channel.send({
        files: ['https://www.institut-repere.com/info/wp-content/uploads/2009/02/pouvoirs-divin-1200x905.jpg']
      });
    }
    ...
    // Affiche un message et une image en fonction de la tranche de 10 dans laquelle se trouve le nombre aléatoire
      ...
      message.channel.send({
        files: ['https://resize-elle.ladmedia.fr/rcrop/638,,forcex/img/var/plain_site/storage/images/people/la-vie-des-people/news/comment-tomber-amoureuse-de-james-franco-en-20-gifs/le-clin-d-oeil/43591737-1-fre-FR/Le-clin-d-oeil.gif']
      });
    }
      ...
      message.channel.send({
        files: ['https://media.tenor.com/mdWFxHBHuusAAAAM/%ED%98%BC%EB%82%B4%EC%95%BC%EA%B2%A0%EC%96%B4-whip.gif']
      });
  }
    else if (randomNumber >= 70 && randomNumber <= 79) {
      message.channel.send(`Ton score est de : ${randomNumber}`);
      message.channel.send("Tu es proche mais vraiment très très proche mais genre, trèèèèèèèèèèèèèèèèèèèèèèèèès proche de ce qu'on appelle la limite que je me suis accordé pour que je puisse encore ne serais-ce t'adresser la parole... Ou même te bloquer... mais genre vraiment très proche. ")
  }
    else if (randomNumber >= 80 && randomNumber <= 89) {
      message.channel.send(`Ton score est de : ${randomNumber}`);
      message.channel.send("Si je devais te donner un surnom, je te donnerais celui de **pd** car t'es vraiment pd tema le score.")
  }
      ...
      message.channel.send(`Ton score est de : ${randomNumber}`);
      message.channel.send("Ouf t'es pas passé loin... Mais bon, tu es quand même pd")
    }
    else if (randomNumber === 100) {
      message.channel.send(`Ton score est de : ${randomNumber}`);
      message.channel.send("Ah... C'est donc comme ça...")
      message.channel.send("Si tu devais te faire réincarner en une personne dans une autre vie, tu serais surement....")
      message.channel.send({
      ...
      message.channel.send(`Ton score est de : ${randomNumber}`);
      message.channel.send({
        files: ['https://media.tenor.com/EANWDmJZRtcAAAAd/lil-nas-x-montero.gif']
      });
    }
}
});
